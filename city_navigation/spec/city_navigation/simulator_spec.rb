...
describe CityNavigation::Simulator do
  let(:city_grid) { CityNavigation::CityGrid.new(20, 20) }
  subject { CityNavigation::Simulator.new(city_grid) }

  it "places the explorer on to a valid position" do
    expect(CityNavigation::Explorer).to receive(:new)
      .with(0, 0, "NORTH")
      .and_return(double)
  ...
  it "cannot place the explorer on to an invalid position" do
    expect(CityNavigation::Explorer).not_to receive(:new)
  ...
  context "when explorer has been placed" do
    let(:explorer) { instance_double(CityNavigation::Explorer, next_move: [0, 0]) }
  ...
  context "explorer placed at city boundary" do
    before do
      subject.place(0, 20, "NORTH")
    end

    it "cannot move past the city boundary" do
      subject.move
